    ...
    <template v-if="note">
      ...
      <section class="img-section">
        <div class="img-section__head">
          ...
          <span v-if="note.images && note.images.length" class="img-section__count">{{ note.images.length }}</span>
        </div>

        <div v-if="note.images && note.images.length" class="img-grid">
          <div
...
const loadNote = async () => {
  ...
  const found = await db.notes.get(id)
  if (!found) { alert('记事不存在'); router.back(); return }
  note.value = found
}
...
const handleImageUpload = (event) => {
  ...
  for (const file of files) {
    ...
    reader.onload = (e) => {
      compressImage(e.target.result).then((c) => {
        if (!note.value.images) note.value.images = []
        note.value.images.push(c)
      })
    }
...
const saveNote = async () => {
  ...
  try {
    await db.notes.update(note.value.id, JSON.parse(JSON.stringify({
...
.ed-title:focus { outline: none; border-bottom-color: var(--brand); }
.ed-title::placeholder { color: var(--border-strong); font-weight: 700; }
...
.img-section__label { font-size: var(--fs-sm); font-weight: 600; color: var(--text); }
.img-section__count {
  font-size: var(--fs-xs);
...
.viewer__box { position: relative; max-width: 92%; max-height: 88%; }
.viewer__x {
  position: absolute;
  top: -48px;
  right: 0;
